...
contract DSCEngine is ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        _burnDsc(debtToCover, msg.sender, msg.sender);
    ...
    function _calculateHealthFactor(uint256 totalDscMinted, uint256 collateralValueUsd)
        internal
        pure
        returns (uint256) 
    {
    ...
    function getAccountCollateralValue(address user) public view returns (uint256 totalCollateralValueInUsd) {
        for (uint256 index = 0; index < s_collateralTokens.length; index++) {
            ...
            console.log("User usdValue: ", totalCollateralValueInUsd);
        }
        return totalCollateralValueInUsd;
    }

    function getUsdValue(address token, uint256 amount) external view returns (uint256) {
        console.log("getUsdValue token: ", token);
    ...
    function _getUsdValue(address token, uint256 amount) private view returns (uint256) {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(s_priceFeeds[token]);
        (, int256 price,,,) = priceFeed.latestRoundData();
        console.log("_getUsdValue token: ", token);
        console.log("_getUsdValue amount: ", amount);
        console.log("_getUsdValue price: ", price);
